...
# Selalu resolve path relatif terhadap lokasi file app.py ini,
# bukan terhadap current working directory saat proses Streamlit dijalankan
# (di Streamlit Cloud, cwd bisa berbeda dari folder repo -> menyebabkan FileNotFoundError).
BASE_DIR = Path(__file__).resolve().parent
MODEL_DIR = BASE_DIR / "model"

@st.cache_resource
def load_artifacts():
    missing = [f for f in ["model.joblib", "scaler.joblib", "label_encoder.joblib", "feature_names.joblib"]
               if not (MODEL_DIR / f).exists()]
    if missing:
        st.error(
            f"File model tidak ditemukan di `{MODEL_DIR}`: {missing}. "
            "Pastikan folder `model/` sudah ikut di-push ke repository GitHub "
            "(cek juga file `.gitignore`, siapa tahu folder `model/` ikut terabaikan)."
        )
        st.stop()
